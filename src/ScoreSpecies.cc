...
double ScoreSpecies::fCurrentLET = 0.;
...
{
  std::vector<G4String> targetSpecies = {
    "H3O^1", "OH^0", "OH^-1", "e_aq^-1", "H^0", "H_2^0", "H2O2^0", "O^0"
  };

  std::map<G4double, std::map<G4String, double>> speciesGPerTime;

  std::map<G4double, std::map<G4String, double>> speciesGPerLET;
  ...
  std::ofstream csvFile("Gvalues_species.csv");
  csvFile << "Time,Nevent";
  for(const auto& sp : targetSpecies) {
    csvFile << "," << sp;  
  }
  csvFile << "\n";

  std::ofstream csvLET("Gvalues_LET.csv");
  csvLET << "LET,Nevent";
  for(const auto& sp : targetSpecies) {
    csvLET << "," << sp;
  }
  ...
  G4cout << "[WriteWithAnalysisManager] Entry count: " << fSpeciesInfoPerTime.size() << G4endl;
  ...
  for(const auto& it_map1 : fSpeciesInfoPerTime)
  {
    ...
    for(const auto& it_map2 : map2)
    {
      ...
      if(std::find(targetSpecies.begin(), targetSpecies.end(), name) != targetSpecies.end())
      {
        double G = it_map2.second.fG;
        double LET = it_map2.second.fLET;
        speciesGPerTime[time][name] = G;
        speciesGPerLET[LET][name] = G;
      }
    }
  }

  for(const auto& timeEntry : speciesGPerTime) {
    double time = timeEntry.first;
    csvFile << time << "," << fNEvent;

    for(const auto& sp : targetSpecies) {
      auto it = timeEntry.second.find(sp);
      if(it != timeEntry.second.end()) {
        csvFile << "," << it->second;
      } else {
        csvFile << ",";
      }
  ...
  for(const auto& letEntry : speciesGPerLET) {
    double LET = letEntry.first;
    csvLET << LET << "," << fNEvent;

    for(const auto& sp : targetSpecies) {
      auto it = letEntry.second.find(sp);
      if(it != letEntry.second.end()) {
        csvLET << "," << it->second;
      } else {
        csvLET << ",";
      }
  ...
  auto it_first_map1 = fSpeciesInfoPerTime.begin();
  if(it_first_map1 != fSpeciesInfoPerTime.end())
  {
    const InnerSpeciesMap& map2 = it_first_map1->second;
    for(const auto& it_molname : map2){
      auto tmp_species = it_molname.first;
      G4cout << std::setw(12) << tmp_species->GetName()
    ...
    for(const auto& it_map1: fSpeciesInfoPerTime){
      ...
      for(const auto& it_map2 : map2)
      {
        auto species = it_map2.first;
        const G4String& name = species->GetName();
        int molID = species->GetMoleculeID();
        ...
        if(time == *fTimeToRecord.rbegin()){
          if(N > 1){
            G4cout<<std::setw(12)<<G/N<<std::setw(12)<<
      ...
      //G4cout << molID << "," << number << "," << fNEvent << "," << name << "," << time << "," << G << "," << G2 << G4endl;
      //csvFile << molID << "," << number << "," << fNEvent << "," << name << "," << time << "," << G << "," << G2 << "\n";
