...
void ScoreLET::Output(std::ostream& out)
{
  WriteLETCSV(); 
  out << "LET values by species have been written to LET_by_species.csv\n";
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo.....

void ScoreLET::WriteLETCSV()
{
  std::ofstream file("LET_species.csv");
  if(!file.is_open()) return;

  file << "Event";
  const std::vector<std::string> speciesOrder = {
    "H3O^1", "OH^0", "OH^-1", "e_aq^-1", "H^0", "H_2^0", "H2O2^0", "O^0"
  };
  for(const auto& s : speciesOrder){
    file << "," << s;
  }
  file << "\n";

  size_t maxEvents = 0;
  for(const auto& p : fLETBySpecies){
    if(p.second.size() > maxEvents)
      maxEvents = p.second.size();
  }

  for(size_t i = 0; i < maxEvents; ++i){
    file << i;
    for(const auto& s : speciesOrder){
      auto it = fLETBySpecies.find(s);
      if(it != fLETBySpecies.end() && i < it->second.size()){
        file << "," << std::fixed << std::setprecision(6) << it->second[i];
      } else {
        file << ",";
      }
    }
